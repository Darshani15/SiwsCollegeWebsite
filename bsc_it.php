<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>B.Sc. Information Technology</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            margin: 0;
            padding: 0;
        }

        /* Tab styling */
        .tabs {
            display: flex;
            margin-bottom: 20px;
            background-color: #f1f1f1;
        }

        .tabs button {
            padding: 10px;
            margin-right: 10px;
            background-color: #4CAF50;
            color: white;
            border: none;
            cursor: pointer;
            font-size: 16px;
        }

        .tabs button:hover {
            background-color: #45a049;
        }

        .tabs button.active {
            background-color: #45a049;
            font-weight: bold;
        }

        /* Tab content styling */
        .tab-content {
            display: none;
            padding: 20px;
            border: 1px solid #ddd;
            border-top: none;
        }

        .tab-content table {
            width: 100%;
            border-collapse: collapse;
        }

        .tab-content th, .tab-content td {
            border: 1px solid #ddd;
            padding: 10px;
            text-align: center;
        }

        .tab-content th {
            background-color: #4CAF50;
            color: white;
        }

        .tab-content tr:nth-child(even) {
            background-color: #f2f2f2;
        }

        .tab-content tr:last-child {
            font-weight: bold;
        }

        .program-profile {
            margin-bottom: 20px;
            font-size: 18px;
        }

        .program-profile p {
            margin: 10px 0;
        }
    </style>
</head>
<body>

    <!-- Program Profile Information -->
    <div class="program-profile">
        <h1>B.Sc. Information Technology</h1>
        <p><strong>Duration:</strong> 6 semesters</p>
        <p><strong>Intake:</strong> 60</p>
        <p><strong>Eligibility:</strong> A student must have passed Std. XII (Higher Secondary Certificate Examination) conducted by any statutory board with a minimum of 45% Passing (Open) and 40% (Reserved) in First Attempt with Mathematics as a subject.</p>
    </div>

    <!-- Tabs for each semester -->
    <div class="tabs">
        <button class="tablink active" onclick="openTab(event, 'sem1')">Semester 1</button>
        <button class="tablink" onclick="openTab(event, 'sem2')">Semester 2</button>
        <button class="tablink" onclick="openTab(event, 'sem3')">Semester 3</button>
        <button class="tablink" onclick="openTab(event, 'sem4')">Semester 4</button>
        <button class="tablink" onclick="openTab(event, 'sem5')">Semester 5</button>
        <button class="tablink" onclick="openTab(event, 'sem6')">Semester 6</button>
    </div>

    <!-- Tab Content for Semesters -->
    <div id="sem1" class="tab-content active">
        <h3>Semester 1</h3>
        <!-- Semester 1 Table -->
        <table>
            <thead>
                <tr>
                    <th>Sr. No.</th>
                    <th>Courses</th>
                    <th>Title</th>
                    <th>Credits</th>
                    <th>Lectures / per week</th>
                    <th>Syllabus</th>
                    <th>Examination Patterns</th>
                    <th>E-content / Notes</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>1</td>
                    <td>Major</td>
                    <td>Introduction to IT & Algorithms</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td>2</td>
                    <td>Minor</td>
                    <td>Discrete Structures</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td>3</td>
                    <td>SEC</td>
                    <td>Front End Technologies</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td>4</td>
                    <td>AEC</td>
                    <td>Communication Skills in English -I</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td>5</td>
                    <td>VEC</td>
                    <td>Constitution of India - I</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td>6</td>
                    <td>OEI</td>
                    <td>Personal Banking</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td>7</td>
                    <td>OE2</td>
                    <td>Introduction to Advertising</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td>8</td>
                    <td>IKS</td>
                    <td>Indian Knowledge System</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td>9</td>
                    <td>CC (ANY ONE)</td>
                    <td>NCC</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td></td>
                    <td>NSS</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td></td>
                    <td>DLLE</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td></td>
                    <td>Dramatics</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td></td>
                    <td>Fine Art</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td></td>
                    <td>Yoga</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr>
                    <td></td>
                    <td>Health & Wellness</td>
                    <td>2</td>
                    <td>2</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
                <tr style="background-color: #f9f9f9; font-weight: bold;">
                    <td colspan="3">Total</td>
                    <td>22</td>
                    <td>22</td>
                    <td>---</td>
                    <td>---</td>
                    <td>---</td>
                </tr>
            </tbody>
        </table>
    </div>

    <div id="sem2" class="tab-content active">
        <h3>Semester 2</h3>
        <!-- Semester 2 Table -->
         <table>
    <thead>
        <tr>
            <th>Sr. No.</th>
            <th>Courses</th>
            <th>Title</th>
            <th>Credits</th>
            <th>Lectures / per week</th>
            <th>Syllabus</th>
            <th>Examination Patterns</th>
            <th>E-content / Notes</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Introduction to Operating Systems</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Minor</td>
            <td>Introduction to Database Management Systems</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>3</td>
            <td>VSC</td>
            <td>UI/UX - Dart Programming</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>4</td>
            <td>SEC</td>
            <td>Front End Technologies - II (Introduction to Angular)</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>5</td>
            <td>AEC</td>
            <td>Communication Skills in English - II</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>6</td>
            <td>VEC</td>
            <td>Constitution of India - II</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>7</td>
            <td>OEI</td>
            <td>Personal Taxation</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>OE2</td>
            <td>Fundamentals of Advertising</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>9</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>NCC</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td></td>
            <td>NSS</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td></td>
            <td>DLLE</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td></td>
            <td>Dramatics</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td></td>
            <td>Fine Art</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td></td>
            <td>Yoga</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td></td>
            <td>Health & Wellness</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr style="background-color: #f9f9f9; font-weight: bold;">
            <td colspan="3">Total</td>
            <td>22</td>
            <td>22</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
    </tbody>
</table>
    </div>
        <div id="sem3" class="tab-content active">
        <h3>Semester 3</h3>
        <!-- Semester 3 Table -->
         <table>
    <thead>
        <tr>
            <th>Sr. No.</th>
            <th>Courses</th>
            <th>Title</th>
            <th>Credits</th>
            <th>Lectures / per week</th>
            <th>Syllabus</th>
            <th>Examination Patterns</th>
            <th>E-content / Notes</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Data Communications and Networking</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Computer Graphics</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Information Technology Major Practical -III</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Minor</td>
            <td>Computational Mathematics</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Minor</td>
            <td>Internet Technologies</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Minor</td>
            <td>Information Technology Minor Practical -III</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>3</td>
            <td>VSC</td>
            <td>Gates, Instruction Set and Programming</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>4</td>
            <td>SEC</td>
            <td>JAVA Programming</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>5</td>
            <td>AEC</td>
            <td>Communication Skills in Hindi -II</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>6</td>
            <td>AEC</td>
            <td>Communication Skills in Marathi -II</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>7</td>
            <td>OE</td>
            <td>Cyber Laws & Corporate Laws</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>NCC</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>NSS</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>DLLE</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>Dramatics</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>Fine Art</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>Yoga</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>Health & Wellness</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr style="background-color: #f9f9f9; font-weight: bold;">
            <td colspan="3">Total</td>
            <td>22</td>
            <td>22</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
    </tbody>
</table>
    </div>
            <div id="sem4" class="tab-content active">
        <h3>Semester 4</h3>
        <!-- Semester 4 Table -->
         <table>
    <thead>
        <tr>
            <th>Sr. No.</th>
            <th>Courses</th>
            <th>Title</th>
            <th>Credits</th>
            <th>Lectures / per week</th>
            <th>Syllabus</th>
            <th>Examination Patterns</th>
            <th>E-content / Notes</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Software Engineering</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Introduction to Virtual Reality</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Information Technology Major Practical IV</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Information Technology Major Practical V</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Minor</td>
            <td>Introduction to Data Mining</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Minor</td>
            <td>Information Technology Minor Practical IV</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>3</td>
            <td>VSC</td>
            <td>Introduction to Embedded Systems</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>4</td>
            <td>CEP</td>
            <td>Community Engagement Service Project</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>5</td>
            <td>AEC</td>
            <td>Communication Skills in Hindi -II</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>6</td>
            <td>AEC</td>
            <td>Communication Skills in Marathi -II</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>7</td>
            <td>OE</td>
            <td>Intellectual Property Laws</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>NCC</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>NSS</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>DLLE</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>Dramatics</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>Fine Art</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>Yoga</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>8</td>
            <td>CC (Same as in Previous Sem)</td>
            <td>Health & Wellness</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr style="background-color: #f9f9f9; font-weight: bold;">
            <td colspan="3">Total</td>
            <td>22</td>
            <td>22</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
    </tbody>
</table>
    </div>
                <div id="sem5" class="tab-content active">
        <h3>Semester 5</h3>
        <!-- Semester 5 Table -->
         <table>
    <thead>
        <tr>
            <th>Sr. No.</th>
            <th>Courses</th>
            <th>Title</th>
            <th>Credits</th>
            <th>Lectures / per week</th>
            <th>Syllabus</th>
            <th>Examination Patterns</th>
            <th>E-content / Notes</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Software Project Management</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Distributed Systems</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Information Systems Security</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Introduction to Digital Image Processing</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Indian Nyaya in Western Dialect</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Information Technology Major Practical VI</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>1</td>
            <td>Major</td>
            <td>Information Technology Major Practical VII</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Elective</td>
            <td>Big Data Frameworks</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Elective</td>
            <td>Information Technology Elective Practical</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>3</td>
            <td>VSC</td>
            <td>Server-Side Programming</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr>
            <td>4</td>
            <td>FP</td>
            <td>Field Project</td>
            <td>2</td>
            <td>2</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
        <tr style="background-color: #f9f9f9; font-weight: bold;">
            <td colspan="3">Total</td>
            <td>22</td>
            <td>22</td>
            <td>---</td>
            <td>---</td>
            <td>---</td>
        </tr>
    </tbody>
</table>
    </div>
          <!-- SEMESTER 6 -->
  <div id="sem6" class="tab-content">
    <h2 class="text-2xl font-semibold mb-4">Semester 6</h2>
    <p class="text-gray-600">Content coming soon...</p>
  </div>
                              <!-- Footer -->
    <footer>
        <div class="footer-content">
            <p>SIWS N.R. Swamy College of Commerce & Economics and Smt. Thirumalai College of Science (Autonomous)</p>
            <p>Wadala, Mumbai 400031 | Phone: +91-22-24143335 | Email: info@siwscollege.edu.in</p>
            <p>© 2023 SIWS College. All Rights Reserved.</p>
        </div>
    </footer>


    <script>
        // Function to open the selected tab
        function openTab(evt, tabName) {
            var i, tabcontent, tablinks;

            // Hide all tab contents
            tabcontent = document.getElementsByClassName("tab-content");
            for (i = 0; i < tabcontent.length; i++) {
                tabcontent[i].style.display = "none";
            }

            // Remove "active" class from all tabs
            tablinks = document.getElementsByClassName("tablink");
            for (i = 0; i < tablinks.length; i++) {
                tablinks[i].className = tablinks[i].className.replace(" active", "");
            }

            // Show the clicked tab content
            document.getElementById(tabName).style.display = "block";
            evt.currentTarget.className += " active";
        }

        // Set default active tab
        document.getElementsByClassName("tablink")[0].click();
    </script>

</body>
</html>
